...
public class Car implements Functionalities {
	int current_gear, current_speed;
	
	public Car() {
		current_gear = 1; // initial gear 1
		current_speed = 0; // initial speed 0
	}
	...
	public void apply_brakes() {
		...
		else if((0 <= current_speed && current_speed <= 20)) {
			current_gear = 1;
			current_speed -= 5;
		}
		else if((20 <= current_speed && current_speed <= 40)) {
			current_gear = 2;
			current_speed -= 5;
		}
		else if((40 <= current_speed && current_speed <= 60)) {
			current_gear = 3;
			current_speed -= 5;
		}
		else if((60 <= current_speed && current_speed <= 80)) {
			current_gear = 4;
			current_speed -= 5;
		} else
			current_speed -= 5;
	...
	public void speed_up(int s) {
		...
		if(s > 20 && current_gear == 1)
			System.out.println("\nPlease change gears before speeding up\n");
		else if(s > 40 && current_gear == 2)
			System.out.println("\nPlease change gears before speeding up\n");
		else if(s > 60 && current_gear == 3)
			System.out.println("\nPlease change gears before speeding up\n");
		else if(s > 80 && current_gear == 4)
			System.out.println("\nPlease change gears before speeding up\n");
		else if(s > 100 && current_gear == 5)
			System.out.println("\nYou cannot speed up more\n"); // max speed is set to 100
		else
			current_speed = s;
	}
